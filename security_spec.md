...
- **Invariant 3 (Circulation Integrity)**: Readers/Students can NEVER create, alter, or delete loans (`/emprestimos`) or reader records (`/leitores`). Only library staff (`isProfessor()`) can perform these mutations.
- **Invariant 4 (Role Escalation Prevention)**: Users registering or updating their `/users/{userId}` profile can never self-assign the `professor` or `admin` role without verification. Self-updates cannot mutate `role`.
- **Invariant 5 (Immutable Audit Logs)**: Documents in `/auditoria` can only be created by authenticated users recording their own actions (`usuario_id == request.auth.uid`). Once created, updates and deletions are mathematically forbidden (`allow update, delete: if false;`).
...
- **Invariant 7 (Moderated Social Interaction)**: Reviews in `/comentarios` must have ratings strictly between 1 and 5, max 1000 characters, author ID matching `request.auth.uid`, and only approved reviews are publicly visible.
...
## 2. The "Dirty Dozen" Payloads (Penetration Test Vectors)
...
### Payload 4: Self-Assigned Professor Privilege Escalation
...
- **Expected Outcome**: `PERMISSION_DENIED` (Cannot self-assign `professor` role)
...
### Payload 7: Audit Log Spoofing (Falsifying Author)
- **Target**: `POST /auditoria/aud_spoofed`
- **Auth**: `uid: "student_uid"`
- **Payload**: `{"id": "aud_spoofed", "usuario_id": "different_victim_uid", "usuario_nome": "Victim", "acao": "DELETAR_TUDO", "tabela_afetada": "livros", "detalhes": "x", "criado_em": "2026-03-01"}`
- **Expected Outcome**: `PERMISSION_DENIED` (Cannot forge `usuario_id`)
