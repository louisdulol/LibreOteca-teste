...
export const RelatoriosView: React.FC = () => {
  ...
  const handleExportarBackupJson = () => {
    ...
    setDownloadFeedback('Backup estruturado JSON (Firestore) gerado e baixado com sucesso!');
    setTimeout(() => setDownloadFeedback(null), 4000);
  };

  const handleExportarCsv = (tipo: 'livros' | 'emprestimos' | 'leitores') => {
    if (tipo === 'livros') StorageService.exportarLivrosCsv();
    if (tipo === 'emprestimos') StorageService.exportarEmprestimosCsv();
    if (tipo === 'leitores') StorageService.exportarLeitoresCsv();
    setDownloadFeedback(`Exportação de ${tipo} em CSV concluída!`);
    setTimeout(() => setDownloadFeedback(null), 4000);
  };
  ...
  return (
    <div className="space-y-6 w-full max-w-full min-w-0 overflow-hidden">
      ...
      {/* Seção 3: Central de Backup & Automação de Dados (Google Cloud Firestore) */}
      <div className="p-6 bg-[#131926] rounded-3xl border border-slate-800 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-emerald-950/60 text-emerald-400 flex items-center justify-center border border-emerald-800 shrink-0">
              <Cloud className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-base font-serif font-bold text-white flex items-center gap-2">
                <span>Backup & Automação de Dados (Google Cloud Firestore)</span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                  Online
                </span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Dados protegidos por segurança em nível de documento, sincronização contínua e exportações para automação
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold rounded-xl text-xs shadow-md transition-all active:scale-95 cursor-pointer shrink-0"
          >
        ...
        {/* Resumo das Coleções no Firestore */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
          <div className="p-3.5 bg-slate-950/60 border border-slate-800/80 rounded-2xl">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 text-xs font-mono">/livros</span>
              <BookOpen className="w-3.5 h-3.5 text-amber-400" />
            </div>
            <p className="text-xl font-bold text-white mt-1">{totalLivros}</p>
            <span className="text-[10px] text-slate-500">Documentos de acervo</span>
          </div>

          <div className="p-3.5 bg-slate-950/60 border border-slate-800/80 rounded-2xl">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 text-xs font-mono">/leitores</span>
              <Users className="w-3.5 h-3.5 text-cyan-400" />
            </div>
            <p className="text-xl font-bold text-white mt-1">{leitores.length}</p>
            <span className="text-[10px] text-slate-500">Leitores cadastrados</span>
          </div>

          <div className="p-3.5 bg-slate-950/60 border border-slate-800/80 rounded-2xl">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 text-xs font-mono">/emprestimos</span>
              <Layers className="w-3.5 h-3.5 text-indigo-400" />
            </div>
            <p className="text-xl font-bold text-white mt-1">{emprestimos.length}</p>
            <span className="text-[10px] text-slate-500">Histórico de circulação</span>
          </div>

          <div className="p-3.5 bg-slate-950/60 border border-slate-800/80 rounded-2xl">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 text-xs font-mono">Segurança</span>
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            </div>
            <p className="text-base font-bold text-emerald-300 mt-1">Regras Ativas</p>
            <span className="text-[10px] text-slate-500">Conformidade LGPD</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800/80 text-xs text-slate-400">
          <span className="text-[11px] text-slate-400">
            Exportações em planilha tabulada para relatórios externos e auditoria:
          </span>
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => handleExportarCsv('livros')}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
            >
              <FileSpreadsheet className="w-3.5 h-3.5 text-amber-400" />
              <span>Acervo (CSV)</span>
            </button>
            <button
              onClick={() => handleExportarCsv('emprestimos')}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
            >
            ...
            <button
              onClick={() => handleExportarCsv('leitores')}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
            >
